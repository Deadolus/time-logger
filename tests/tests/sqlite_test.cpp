...
public:
  ...
  ~UniqueDatabase() { std::remove(name_.c_str()); }
...
protected:
  Sqlite sqlite{};
  void SetUp() override { EXPECT_TRUE(sqlite.open(database.getName())); }
...
TEST_F(SqliteTest, can_init_database) { EXPECT_TRUE(sqlite.init()); }
...
TEST_F(SqliteTest, entry_is_correct_after_insert) {
  ...
  std::string sql = "SELECT * FROM devicelog;";
  DatabaseCallback cb = [](std::vector<DatabaseEntry> &entries) {
    // for(auto& entry : entries) { std::cout << entry.first <<
